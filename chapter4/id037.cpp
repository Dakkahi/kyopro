...
int main(){
    ...
    if(val1*val2 < 0) ans = "Yes";
        ...
        else if (val1 == 0){ // 1,3と1,4で平行になったとき
            if(val2 != 0){ //val1 == 0でval2 != 0のとき
                if(scalar_prod(x3-x1, x4-x1, y3-y1, y4-y1) < 0) ans = "Yes"; // 3,1,4の順で点が並んでいるとき
                else ans = "No"; //1,3,4の順で点が並んでいるとき
            }
            //4点が同一線分上にあるとき(val1 == val2 == 0)
            else if(scalar_prod(x3-x1, x3-x2, y3-y1, y3-y2) < 0 || scalar_prod(x4-x1, x4-x2, y4-y1, y4-y2) < 0) ans = "Yes";
            else if(scalar_prod(x3-x1, x4-x1, y3-y1, y4-y1) < 0 || scalar_prod(x3-x2, x4-x2, y3-y2, y4-y2) < 0) ans = "Yes";
            else ans = "No";
        }

        else if(val2 == 0){ //val1 != 0でval2 == 0のとき
